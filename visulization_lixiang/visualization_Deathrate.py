...
countries_of_the_world.Deathrate.replace(['-'],0.0,inplace = True)
countries_of_the_world.Deathrate = countries_of_the_world.Deathrate.astype(float)
area_list = list(countries_of_the_world['Country'].unique())
area_poverty_ratio = []
for i in area_list:
    x = countries_of_the_world[countries_of_the_world['Country']==i]
    area_poverty_rate = sum(x.Deathrate)/len(x)
...
plt.title('Deathrate of Countries')