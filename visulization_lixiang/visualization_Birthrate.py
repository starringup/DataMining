...
countries_of_the_world.Birthrate.replace(['-'],0.0,inplace = True)
countries_of_the_world.Birthrate = countries_of_the_world.Birthrate.astype(float)
area_list = list(countries_of_the_world['Country'].unique())
area_poverty_ratio = []
for i in area_list:
    x = countries_of_the_world[countries_of_the_world['Country']==i]
    area_poverty_rate = sum(x.Birthrate)/len(x)
...
plt.title('Birthrate of Countries')